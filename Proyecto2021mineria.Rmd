...
sanguchez$mah = mahalanobis(cbind(Precio/max(Precio),nota/max(nota),suma1/max(suma1), suma2/max(suma2)) ,
                            colMeans(cbind(Precio/max(Precio),nota/max(nota),suma1/max(suma1), suma2/max(suma2))),
                            cov(cbind(cbind(Precio/max(Precio),nota/max(nota),suma1/max(suma1), suma2/max(suma2)))))
...
ggplot(DF, aes(x=c(1:358),y=sanguchez$mah,color=sanguchez$mah)) +
...
tempDist <- distances(DF[,c(4,6,11,12)],normalize = "mahalanobize") %>% as.dist()
...
ggplot(DF, aes(x=c(1:358),y=temp,color=temp)) +
  geom_point(size=1,alpha=0.8)+theme_bw()+
  scale_color_gradient(low="blue",high="red")+
  ggtitle("4-NN") + 
  theme_bw() +
...
#preservo las 4 distancias menores
temp <- kNNdist(tempDist, 5, all=T)
temp <- temp[,5] # preservo la maxima de las 4 distancias cercanas
...
```{r}
df_5 = filter(DF, sanguchez$mah < 10, temp<1.5)
dim(DF)
dim(df_5)
```
